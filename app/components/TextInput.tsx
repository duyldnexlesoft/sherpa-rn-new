...
const InputComponent = (props: any) => {
  ...
  return (
    <View
      className={
        'w-full flex-row items-center justify-between rounded-md border bg-white ' + cssBorder()
      }
      ...
      {props.leftIcon && (
        <Pressable
          onLayout={(event) =>
            setWidthLeftIcon((event.nativeEvent.layout.width || 0) + PADDING_LEFT)
          }
      ...
      {props.label && (
        <Animated.Text
          className='absolute top-4 rounded-md bg-white px-2 text-gray-500'
      ...
      {props.rightIcon && (
        <Pressable
          className={`flex h-12 items-center justify-center p-2 ${props.rightAction ? 'z-30' : ''}`}
...
const TextInput = (props: InputProps) => {
  return props.control ? (
    <Controller
      ...
      rules={props.rules}
      render={({ field }) => <InputComponent field={field} {...props} />}
      name={props.name}
    />
  ) : (
